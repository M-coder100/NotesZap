...
function renderNotes(notes: object[]) {
    ...
    notes.forEach((item: any) => {
        ...
        classes[0] ? note.classList.add(classes[0]) : false; classes[1] ? note.classList.add(classes[1]) : false
        $(note).HTML(`
            <div class="topBar">
                <div class="grp">
...
function noteControl() {
    if (!!document.querySelector("note")) {
        $`note`.each((element: Element) => {
            $(".topBar .option").each((option: Element) => {
                option.addEventListener("click", () => {
                    (function takeActionForTheNote(opt: string, index: string) {
